...
public class PolymorphismPractice2 {
    ...
    public static void main(String[] args) {
        ...
        //2.2 & 2.3store all the testers into the following List of testers:
        ...
        for (Employee scrumMember : scrumMembers) {
            if (scrumMember instanceof Tester){
                testers.add((Tester) scrumMember); // scrumMember is Employee class here, so you gotta assign employee to Tester
                //so you gotta assign parent to child. so this is downcasting. so you gotta explicitly cast
        ...
        Tester testerWithHighestSalary = testers.get(0);

        for (Tester eachTester : testers) {
            if (eachTester.getSalary() > testerWithHighestSalary.getSalary()){
                testerWithHighestSalary = eachTester;
            }
        }

        System.out.println(testerWithHighestSalary);

        System.out.println("---------------------------------------------");

        Developer devWithHighestSalary = developers.get(0);

        for (Developer eachDeveloper : developers) {
            if (eachDeveloper.getSalary() > devWithHighestSalary.getSalary()){
                devWithHighestSalary = eachDeveloper;
            }
        }

        System.out.println(devWithHighestSalary);
